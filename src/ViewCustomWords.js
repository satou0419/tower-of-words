...
function ViewCustomWords() {
  ...
  useEffect(() => {
    ...
    .then(data => {
        ...
        const filteredWords = data.filter(word => selectedWordsIds.includes(word.wordID));
    ...
    <>
      <div className="main-viewcustomwords">
        <div className="viewcustomwords-top-container">
          <button className="btn-back btn-back-viewcustomwords" onClick={goBack}>BACK</button>
        </div>
