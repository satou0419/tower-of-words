...
function Shop() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:8080/watataps/items/getAllItems")
      .then((response) => {
        ...
        if (userCredit >= selectedItem.price) {
          try {
            ...
            const userItem = userDetails.userItems.find(
              (userItem) => userItem.item.itemID === selectedItem.itemID
            );
            if (userItem) {
              ...
              await axios.put(
                `http://localhost:8080/watataps/userDetails/update/${userId}`,
                {
          ...
          setShowNotEnoughCreditDialog(true);
        }
      } else {
    ...
    <>
      ...
      {showSuccessDialog && (
        <DialogBox
          ...
          message={`Purchase successful! Your new credit balance is ${localStorage.getItem(
            "credit"
          )} credits`}
          imageSrc={selectedItem.imagePath}
          buttons={[
      ...
      {showNotEnoughCreditDialog && (
        <DialogBox
          title="Not Enough Credit"
          message="Sorry you do not have enough credit to purchase this item"
          imageSrc="./images/sad robot.png"
          buttons={[
            {
              label: "Close",
              onClick: handleCloseNotEnoughCreditDialog,
              className: "btn-close",
            },
          ]}
        />
