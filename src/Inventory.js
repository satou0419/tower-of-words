...
function Inventory() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        const response = await axios.get('http://localhost:8080/watataps/items/getAllItems');
  ...
  const handleButtonClick = async (itemId) => {
    try {
      const response = await axios.get(`http://localhost:8080/watataps/items/getItemById/${itemId}`);
        ...
        <div className="main-container-left-inventory">
          <div className="main-border-view-inventory">
            <div className="word-container-scroll-inventory">
              {items.map((item) => {
                const userItem = userDetails?.userItems.find((userItem) => userItem.item.itemID === item.itemID);
                ...
                return (
                  ...
                  >
                    <div className="inner-button-container inventory">
                      ...
                      />
                      <span className="item-quantity">{userItem ? `${userItem.quantity}x` : '0x'}</span>
                    </div>
                    <div className="inventory-item-button-descriptions">
                      <h2 className="item-name-button-inventory">{item.itemName}</h2>
                      <p className="item-button-description-inventory">{item.itemDescription}</p>
                    </div>
                  </button>
                );
              })}
          ...
          <div className="main-border-view-right-inventory">
            <div className="main-container-right-inner-inventory">
              <div className="picture-container-inventory">
                <img src={selectedItem?.imagePath} alt={selectedItem?.itemName} className="inventory-medkit" />
              </div>
              <h2 className="item-name-inventory">{selectedItem?.itemName}</h2>
              <div className="text-container-inventory">
                <p className="item-description-inventory">{selectedItem?.itemDescription}</p>
              </div>
            </div>
          </div>
        </div>
