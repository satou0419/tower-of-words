...
function ViewCustomTower() {
  ...
  const customTowerSearchChange = (event) => {
    ...
    const filtered = customTower.filter((word) =>
  ...
  const participantsSearchChange = (event) => {
    ...
    const filtered = participants.filter((word) =>
      word.toLowerCase().includes(searchTerm.toLowerCase())
    );
            ...
            <div className="main-border-view">
              ...
              <div className="word-container-scroll-viewcustomtower">
                {filteredCustomTower.map((name, index) => (
                  <div
                    key={index}
                    className={`word-container-viewcustomtower ${
                      index === clickedTower ? "clicked" : ""
                    }`}
                    onClick={() => towerClicked(index)}
                  >
                    <h4 className="words-viewcustomtower">{name}</h4>
                  </div>
                ))}
              </div>
            </div>
              ...
              {isRightContainerVisible ? (
                <div className="main-container-right-inner">
                  <div className="main-top-viewparticipants">
                    ...
                    />
                    <button className="btn-next btn-next-viewparticipants">
                      FILTER
                    </button>
                  </div>
                  ...
                  <div className="word-container-scroll-viewparticipants">
                    {filteredParticipants.map((firstname, index) => (
                      <div
                        ...
                        className="word-container-viewcustomtower"
                      >
                        <h4 className="words-viewcustomtower">{firstname}</h4>
                      </div>
                    ))}
                  </div>

                  <div className="code-viewparticipants">CODE: TEST</div>
                </div>
              ) : (
                <div className="main-container-right-blank"></div>
              )}
            </div>
          </div>
        </div>
