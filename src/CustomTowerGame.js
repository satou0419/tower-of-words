...
export default function CustomTowerGame() {
    const handleGetCustomTowerByGameCode = (gamecode) => {
        fetch(`http://localhost:8080/CustomTower/getCustomTowerByGameCode/${gamecode}`)
            .then(response => {
                if (!response.ok) {
            ...
            .then(customTowerData => {
                console.log('CustomTower Data:', customTowerData);
                // Handle the retrieved customTowerData as needed
            })
            .catch(error => {
                console.error('There was a problem fetching the custom tower:', error);
            });
        }
  return (
